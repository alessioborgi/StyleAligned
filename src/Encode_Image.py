...
def images_encoding(model, images: list[np.ndarray], blending_weights: list[float]):
    """
    Encode a list of images using the VAE model and blend their latent representations
    according to the given blending_weights, skipping any images with a weight of 0.0.
    ...
    # Initialize the blended latent representation
    blended_latent_img = torch.zeros_like(valid_latents[0])

    # Perform linear interpolation for all valid latents
    for latent, weight in zip(valid_latents, valid_weights):
        blended_latent_img += latent * weight
