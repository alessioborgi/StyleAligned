...
from .Encode_Image import images_encoding_rbf
...
@torch.no_grad()
def DDIM_Inversion_Process(model: StableDiffusionXLPipeline, x0: list[np.ndarray], blending_weights: list[float], prompt: str, num_inference_steps: int, guidance_scale, epsilon: float = 1.0) -> T:

    # 1) Encode Image: Encode the input image into a latent representation using the model's VAE with RBF interpolation.
    encoded_img = images_encoding_rbf(model, x0, blending_weights, epsilon=epsilon)
